...
// Uncomment to enable debugging of the RunnableTTS class.
...
namespace IBM.Watson.DeveloperCloud.Utilities
{
    ...
    /// </summary>
    public class RunnableTTS : MonoBehaviour
    {
        #region Public Properties
        /// <summary>
        /// Returns the RunnableTTS instance.
        /// </summary>
        public static RunnableTTS Instance { get { return Singleton<RunnableTTS>.Instance; } }
        #endregion
...
#if UNITY_EDITOR
        private static bool _editorRunnableTTS = false;

        /// <summary>
        /// This function enables the RunnableTTS in edit mode.
        /// </summary>
        public static void EnableRunnableTTSInEditor()
        {
            if (!_editorRunnableTTS)
            {
                _editorRunnableTTS = true;
                UnityEditor.EditorApplication.update += UpdateRunnableTTS;
            }
        }
        static void UpdateRunnableTTS()
        {
        ...
        private class Routine : IEnumerator
        {
            ...
            public Routine(IEnumerator a_enumerator)
            {
                ...
                RunnableTTS.Instance.StartCoroutine(this);
                Stop = false;
                ID = RunnableTTS.Instance._nextRoutineId++;

                RunnableTTS.Instance._routines[ID] = this;
#if ENABLE_RUNNABLETTS_DEBUGGING
                Log.Debug("RunnableTTS.Routine()", "Coroutine {0} started.", ID ); 
#endif
            }
            ...
            public bool MoveNext()
            {
                ...
                if (!_moveNext)
                {
                    RunnableTTS.Instance._routines.Remove(ID);      // remove from the mapping
#if ENABLE_RUNNABLETTS_DEBUGGING
                    Log.Debug("RunnableTTS.Routine()", "Coroutine {0} stopped.", ID );
#endif
                }
        ...
        /// invoked from an EditorApplication.Update callback so we can use runnableTTS in Editor mode.
        /// </summary>
